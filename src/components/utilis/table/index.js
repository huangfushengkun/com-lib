...
export default {
  ...
  props: {
    ...
    // 表格列ClassName handleCellClassName
    handleCellClassName: {
      type: Function,
      default: () => {}
    }
          ...
          return createElement(
            ...
            cellComponent,
            {
              ...
              on: {
                ...
                fCellDbClick(event, index, row) {
                  if (event.srcElement.tagName === "P") {
                    event.srcElement.className =
                      event.srcElement.className + " none";
                    event.srcElement.nextElementSibling.className = event.srcElement.nextElementSibling.className.replace(
                      /\bnone\b/,
                      ""
                    );
                 ...
                 * */
                fCellInputBlur(event, index, row, rule, column) {
                  const rst = {};
                  /* 判断传入的是否是正则表达式
                                   非正则表达式*/
                  ...
                  if (rule !== undefined && rule !== "") {
                    ...
                    if (
                      ...
                      !rule.test(event.srcElement.value)
                    ) {
                      // 进行正则表达式匹配
                      /\berror_input\b/.test(event.srcElement.className)
                        ? ""
                        : (event.srcElement.className =
                            event.srcElement.className + " error_input"); // 添加失败的输入框标记
                  ...
                  event.srcElement.parentNode.className =
                    event.srcElement.parentNode.className + " none"; // 切换表格项编辑项目
                  event.srcElement.parentNode.previousElementSibling.className = event.srcElement.parentNode.previousElementSibling.className.replace(
                    /\bnone\b/,
                    ""
                  );
                },
                 ...
                 * @params {object} row 数据行数据
                 * */
                fDeleteBtnClick(row, index, handle) {
                  context.props.handleDelete(row, index, handle);
                }
              }
            }
          );
    ...
    return createElement(
      tableComponentI,
      {
        props: {
          ...
          handleCellClassName: context.props.handleCellClassName
        }
      },
